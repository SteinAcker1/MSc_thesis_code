...
#' ---
#' 
#' # Loading packages and setting theme
## -----------------------------------------------------------------------------
library(Seurat)
library(tidyverse)
library(viridis)
...
## -----------------------------------------------------------------------------
samples <- list()
samples[["NC"]] <- readRDS("/Users/steinacker/Masters_thesis_work/data/parkinsons/substantia_nigra_cond/parkinsons_NC.rds")
samples[["PD"]] <- readRDS("/Users/steinacker/Masters_thesis_work/data/parkinsons/substantia_nigra_cond/parkinsons_PD.rds")
outdir <- "/Users/steinacker/Masters_thesis_work/code_final/substantia_nigra_TEs/output/"
...
cell_types_list <- c(
  ...
  "15" = "Neurons",
  "16" = "Microglia",
  "17" = "Microglia" # assignment of 16 and 17 are less certain
)
 
samples[["NC"]] <- AddCellTypes(samples[["NC"]], cell_types_list)
DimPlot(samples[["NC"]], group.by = "cell_type")

samples[["PD"]] <- AddCellTypes(samples[["PD"]], cell_types_list)
DimPlot(samples[["PD"]], group.by = "cell_type")
...
cell_data_bar_L1s <- cell_data %>%
  ...
  pivot_longer(all_of(L1s_of_interest), names_to = "subfamily", values_to = "expression")
